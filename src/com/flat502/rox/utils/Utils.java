...
public class Utils {
    ...
    /**
     * Parses the value of an HTTP <code>Content-Type</code> header and returns the specified character set.
     * <p>
    ...
    /**
     * Parses the value of an HTTP <code>Content-Type</code> header and returns the Internet Media Type type and
     * sub-type separated by a forwards-slash.
     * <p>
    ...
    /**
     * Converts an {@link java.io.InputStream} containing XML into a {@link Reader} using the encoding specified in
     * the XML header.
     * <p>
     ...
     * @param hint
     *            An optional hint (may be <code>null</code>) if the caller has an idea of what the encoding may be.
     * @return A {@link Reader} instance backed by the given {@link java.io.InputStream} using the encoding
     *         specified by the XML document within the stream.
     * @throws Exception
     *             if the underlying encoding cannot be determined, or if an error occurs while constructing a
     *             reader to return.
     */
        ...
        while (true) {
            ...
            if (--maxChars == 0) {
                throw new UnsupportedEncodingException(
                        "Unable to locate XML header in inputstream within 200 chars");
            }
        }
    }
    ...
    public static <T> T toArray(Class<T> targetType, Object value) {
        if (targetType == Object[].class) {
            return (T) ((List<?>) value).toArray();
        } else if (targetType == int[].class) {
            return (T) toIntArray((List<Integer>) value);
        } else if (targetType == long[].class) {
            return (T) toLongArray((List<Integer>) value);
        } else if (targetType == boolean[].class) {
            return (T) toBooleanArray((List<Boolean>) value);
        } else if (targetType == float[].class) {
            return (T) toFloatArray((List<Double>) value);
        } else if (targetType == double[].class) {
            return (T) toDoubleArray((List<Double>) value);
        }
        ...
        return (T) ((List<?>) value).toArray(arrayTemplate);
    }
    ...
    public static boolean[] toBooleanArray(List<Boolean> list) {
        ...
        Iterator<Boolean> ints = list.iterator();
        int idx = 0;
        while (ints.hasNext()) {
            Boolean value = ints.next();
    ...
    // TODO: Makes unsafe assumptions about the type of "value" (it should be a string for several types,
    // but a List<?> if targetType is an array)
    @SuppressWarnings("unchecked")
    public static <T> T convert(Object value, Class<T> targetType) {
        ...
        if (targetType.isArray()) {
            return convertToArray(targetType, (List<?>) value);
        }
        return (T) value;
    }

    @SuppressWarnings("unchecked")
    public static <T> T convertToArray(Class<T> targetType, List<?> value) {
        if (targetType == Object[].class) {
            return (T) ((List<?>) value).toArray();
        } else if (targetType == int[].class) {
            return (T) convertToIntArray((List<String>) value);
        } else if (targetType == long[].class) {
            return (T) convertToLongArray((List<String>) value);
        } else if (targetType == boolean[].class) {
            return (T) convertToBooleanArray((List<String>) value);
        } else if (targetType == float[].class) {
            return (T) convertToFloatArray((List<String>) value);
        } else if (targetType == double[].class) {
            return (T) convertToDoubleArray((List<String>) value);
        }
        // value is a List with members of the appropriate type
        Object[] arrayTemplate = (Object[]) Array.newInstance(targetType.getComponentType(), 0);
        return (T) ((List<?>) value).toArray(arrayTemplate);
    ...
    public static int[] convertToIntArray(List<String> list) {
        int[] array = new int[list.size()];
        Iterator<String> ints = list.iterator();
        int idx = 0;
        while (ints.hasNext()) {
            array[idx++] = Integer.parseInt(ints.next());
        }
        return array;
    }

    public static long[] convertToLongArray(List<String> list) {
        long[] array = new long[list.size()];
        Iterator<String> ints = list.iterator();
        int idx = 0;
        while (ints.hasNext()) {
            array[idx++] = Long.parseLong(ints.next());
        }
        return array;
    }

    public static boolean[] convertToBooleanArray(List<String> list) {
        boolean[] array = new boolean[list.size()];
        Iterator<String> ints = list.iterator();
        int idx = 0;
        while (ints.hasNext()) {
            array[idx++] = Boolean.parseBoolean(ints.next());
        }
        return array;
    }

    public static float[] convertToFloatArray(List<String> list) {
        float[] array = new float[list.size()];
        Iterator<String> ints = list.iterator();
        int idx = 0;
        while (ints.hasNext()) {
            array[idx++] = Float.parseFloat(ints.next());
        }
        return array;
    }
    ...
    public static double[] convertToDoubleArray(List<String> list) {
        ...
        int idx = 0;
        while (ints.hasNext()) {
            array[idx++] = Double.parseDouble(ints.next());
        }
        return array;
    }
    ...
    public static java.security.cert.X509Certificate convert(javax.security.cert.X509Certificate cert)
            throws javax.security.cert.CertificateEncodingException, java.security.cert.CertificateException {
