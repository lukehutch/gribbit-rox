...
public abstract class HttpMessageBuffer {
     ...
     * @param offset
     *            The offset within <code>newData</code> to begin copying from.
     * @param count
     *            The number of bytes from <code>newData</code> to copy.
     * @return <code>-1</code> if the message is incomplete, <code>0</code> if it is complete and a positive integer
     *         if the message is complete and additional data remains in the input buffer. In the latter case the
     *         returned value is the index into newData at which the excess data begins.
     * @throws Exception
     ...
     * <p>
     * This method returns an unmodifiable {@link Map} containing the headers in the underlying HTTP message. This
     * map's key set iterator will return the keys in the order in which they appeared in the request.
     * <p>
    ...
    /**
     * This is more interesting. We need to find the content-length (mandatory header according to the XMLRPC spec).
     ...
     * <p>
     * This is somewhat cosmetic in that it is used to ensure consistent access to the underlying {@link Map} of
     * headers without producing headers that are unappealing. Most documentation uses headers like "Content-Type"
     * and this method aims to ensure that headers are formatted using this convention.
     * <p>
