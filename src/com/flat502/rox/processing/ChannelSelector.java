...
class ChannelSelector implements Runnable {
    ...
    private void checkForClosure(SelectionKey key) {
        ...
        if (log.logTrace()) {
            log.trace("Checking for possible closure of pooled connection "
                    + System.identityHashCode(socketChannel.socket()));
        }

        if (socketChannel.isConnectionPending()) {
            if (log.logTrace()) {
                log.trace("Connection pending on pooled connection"
                        + System.identityHashCode(socketChannel.socket()));
            }
            return;
        }
        ...
        try {
            ...
            if (log.logTrace()) {
                log.trace(
                        "Connection pending on pooled connection" + System.identityHashCode(socketChannel.socket()),
                        e);
            }
            return;
        } catch (IOException e) {
